...
func removeConn(slice []*websocket.Conn, elem *websocket.Conn) []*websocket.Conn {
	newSlice := make([]*websocket.Conn, len(slice)-1)
...
func (c *ChatServer) registerServer(ws *websocket.Conn) {
...
func (c *ChatServer) readMessage(ws *websocket.Conn) {
	var message string
	for {
		err := websocket.Message.Receive(ws, &message)
		if err != nil {
			c.serverConnPool.unregister <- ws
			log.Println(err)
			return
		}
...
func (c *ChatServer) broadcast(message string) (err error) {
	for _, ws := range c.serverConnPool.connections {
...
func (c *ChatServer) Run() {
	// Listing ConnPool.
	go c.serverConnPool.execute()
	// TODO: Maybe support "/register" to a custom setting.
	// websocket handling.
	http.Handle("/register", websocket.Handler(c.registerServer))
